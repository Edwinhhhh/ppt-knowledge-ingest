...
- Local SQLite database generated by the Web UI server.
- Indexes processed deck metadata and slide-level chunks for local full-text search.
- Uses SQLite FTS5 with a `trigram` tokenizer when available, which works well for Chinese substring-style queries without a separate tokenizer.
- Search results preserve deck folder, slide number, source path or `source_url`, and links to `content.html` / `rendered.html` when available.
- It is a derived artifact and can be safely deleted or rebuilt from `catalog.jsonl` plus per-deck `chunks.jsonl`.
...
all_chunks.jsonl
library_manifest.json
library_index.sqlite  # generated by the local Web UI search indexer
decks/
```
